    ...
    <main className="min-h-screen bg-[#080b12] px-6 py-8 text-white">
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <header className="flex items-center justify-between border-b border-white/10 pb-6">
          ...
          >
            GAME<span className="text-green-400">
              ARENA
            </span>
          </a>
...
{isCaptain && squad.join_code && (
  <div className="mt-5 rounded-2xl border border-green-400/20 bg-green-400/5 p-5">
    ...
    <div className="mt-2 flex items-center justify-between gap-4">
      <p className="text-2xl font-black tracking-[0.25em] text-green-400">
        {squad.join_code}
      </p>
      ...
      <button
        ...
        className="rounded-lg border border-green-400/20 px-3 py-2 text-sm font-bold text-green-400 transition hover:bg-green-400/10"
      >
        Copy
      </button>
    </div>
    ...
    <p className="mt-2 text-sm text-gray-500">
      Share this code with players who want to join your squad.
    </p>
  </div>
)}

{squad.is_winner && (
  <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-yellow-400/20 bg-yellow-400/10 px-4 py-2 text-sm font-black text-yellow-400">
    🏆 TOURNAMENT WINNER
  </div>
)}

          <p className="mt-3 text-gray-400">
            Manage your squad and register all 4
            players.
          </p>
        ...
        <section className="mb-6 rounded-2xl border border-green-400/20 bg-green-400/5 p-6">
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex h-11 w-11 items-center justify-center rounded-full bg-green-400 font-black text-black">
                        {slot}
                      </div>
...
<p className="text-sm text-gray-500">
  ...
  {player.game_id}
                   ...
                   {player &&
  (squad.captain_id === currentUserId ||
    player.user_id === currentUserId) && (
                        <button
                          onClick={() =>
                            removePlayer(player.id)
                          }
                          className="rounded-lg border border-red-400/20 px-3 py-2 text-sm font-bold text-red-400 transition hover:bg-red-400/10"
                        >
                          Remove
                        </button>
                      )}
                  ...
                  {player && (
                    <div className="mt-4 rounded-xl border border-white/10 bg-[#10151f] p-4">
                      ...
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-xs text-gray-500">
                            Upload a clear screenshot showing the player's BGMI name and ID.
                          </p>

                        </div>

                       {player.user_id === currentUserId && (
  <label className="cursor-pointer rounded-xl bg-green-400 px-4 py-3 text-center text-sm font-black text-black transition hover:bg-green-300">
  ...
  {player.id_screenshot_url ? (
    <>
      <span className="rounded-full border border-green-400/20 bg-green-400/10 px-3 py-1.5 text-xs font-bold text-green-400">
        ✓ Screenshot uploaded
      </span>
      ...
      {player.verification_status === "approved" ? (
        ...
        <span className="rounded-full border border-red-400/20 bg-red-400/10 px-3 py-1.5 text-xs font-bold text-red-400">
          ✕ ID Rejected
        </span>
      ) : (
      ...
      Screenshot Pending
                      ...
                      {player.user_id !== squad.captain_id && (
                        <p className="mt-3 text-xs text-gray-500">
                          This player must upload their own ID screenshot from their GameArena account.
                        </p>
                      )}
  ...
  >
    ...
    {submission.status === "rejected" &&
      submission.organizer_note && (
        <div className="mt-4 rounded-lg border border-red-400/10 bg-black/20 p-3">
          <p className="text-xs font-bold uppercase tracking-wider text-gray-500">
            Organizer Note
          </p>
  ...
  <p className="mt-2 text-sm text-gray-400">
    After the match, enter the kills for each squad player and upload the final result screenshot.
  </p>
  ...
  <div className="mt-6 space-y-4">
    ...
    {players.map((player) => (
      <div
        key={player.id}
        className="rounded-xl border border-white/10 bg-[#10151f] p-4"
      >
...
{player.user_id === squad.captain_id && (
  <span className="inline-flex rounded-lg bg-yellow-400/10 px-2.5 py-1 text-xs font-black text-yellow-400">
    👑 Captain
  </span>
)}
  </div>
...
<input
  ...
  onChange={(event) => {
    ...
    setPlayerKills((currentKills) => ({
      ...currentKills,
      [player.id]: value,
    }));
  }}
  placeholder="Kills"
  className="w-24 rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-center text-sm font-bold text-white outline-none focus:border-green-400"
/>
  ...
  <div className="mt-6 rounded-xl border border-white/10 bg-[#10151f] p-4">
    ...
    <p className="mt-1 text-xs text-gray-500">
      Upload the screenshot showing the final match result.
    </p>

    <label className="mt-4 block cursor-pointer rounded-xl border border-dashed border-green-400/30 bg-green-400/5 p-5 text-center text-sm font-bold text-green-400 transition hover:bg-green-400/10">
...
<input
  ...
  onChange={(event) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      setError("Result screenshot must be 10MB or smaller.");
      return;
    }
...
<button
 ...
 disabled={
  ...
  onClick={async () => {
    ...
    if (players.length === 0) {
      setError("Your squad has no registered players.");
      return;
    }

    if (!resultScreenshot) {
      setError("Please upload the final result screenshot.");
      return;
    }
    ...
    try {
      ...
      if (!user) {
        setError(
          "You must be logged in to submit the match result."
        );
      ...
      if (user.id !== squad.captain_id) {
        setError(
          "Only the squad captain can submit the match result."
        );
        setSubmittingResult(false);
        return;
      }
      ...
      if (uploadError) {
        console.error(
          "Result screenshot upload error:",
          uploadError
        );

        setError(
          `Unable to upload result screenshot: ${uploadError.message}`
        );
      ...
      if (submission?.status === "rejected") {
        ...
        if (updateSubmissionError) {
          console.error(
            "Match submission resubmission error:",
          ...
          setError(
            `Unable to resubmit match result: ${updateSubmissionError.message}`
          );
        ...
        const {
          error: deletePlayerResultsError,
        } = await supabase
        ...
        if (deletePlayerResultsError) {
          console.error(
            "Old player results delete error:",
            deletePlayerResultsError
          );

          setError(
            `Unable to replace old player kills: ${deletePlayerResultsError.message}`
          );
        ...
        const {
          ...
          .from(
            "squad_player_match_results"
          )
          .insert(playerResults);

        if (playerResultsError) {
          console.error(
            "New player results error:",
            playerResultsError
          );

          setError(
            `Unable to save new player kills: ${playerResultsError.message}`
          );
        ...
        setMessage(
          "Match result resubmitted successfully. Waiting for organizer review."
        );
      ...
      if (playerResultsError) {
        console.error(
          "Player match results error:",
          playerResultsError
        );

        await supabase
          .from(
            "squad_match_submissions"
          )
          .delete()
          .eq(
            "id",
        ...
        setError(
          `Unable to save player kills: ${playerResultsError.message}`
        );
      ...
      setMessage(
        "Match result submitted successfully. Waiting for organizer review."
      );
      ...
      console.error(
        "Unexpected match result submission error:",
        submitException
      );

      setError(
        "Something went wrong while submitting the match result."
      );
    }

    setSubmittingResult(false);
  }}
  className="mt-6 w-full rounded-xl bg-green-400 px-5 py-3 text-sm font-black text-black transition hover:bg-green-300 disabled:cursor-not-allowed disabled:opacity-50"
>
{submittingResult
  ? "Submitting..."
  : submission?.status === "pending"
  ...
  : "Submit Match Result"}
        ...
        {isCaptain && !squadFull && (
          <section className="mt-6 rounded-2xl border border-white/10 bg-white/[0.03] p-6">
            ...
            <h2 className="mt-2 text-2xl font-black">
              Register a Squad Member
            </h2>

            <p className="mt-2 text-sm text-gray-400">
              Enter the player's GameArena username.
              The player will then upload their own BGMI ID screenshot.
            </p>
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-2">
              ...
              <div>
                ...
                <select
                  ...
                  className="mt-2 w-full rounded-xl border border-white/10 bg-[#10151f] px-4 py-3.5 text-white outline-none focus:border-green-400"
                >
                  {[1, 2, 3, 4]
                    .filter(
                      (slot) =>
              ...
              <div>
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3.5 text-white outline-none placeholder:text-gray-600 focus:border-green-400"
                />
            ...
            <button
              onClick={addPlayer}
              disabled={saving}
              className="mt-6 w-full rounded-xl bg-green-400 py-4 font-black text-black transition hover:bg-green-300 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {saving
                ? "Adding Player..."
                : "Add Player"}
        ...
        {squadFull && (
          <section className="mt-6 rounded-2xl border border-green-400/20 bg-green-400/5 p-6">
            ...
            <p className="mt-2 text-gray-400">
              All 4 players have been added.
              Each player must upload their own
              BGMI ID screenshot.
            </p>

            <div className="mt-5 rounded-xl bg-black/20 p-4 text-sm text-yellow-400">
              ID verification is pending until the
              organizer reviews the screenshots.
            </div>
